...
class LogisticRegression {
  ...
  test(_testFeatures, _testLabels) {
    const predictions = this.predict(_testFeatures);
    _testLabels = tf.tensor(_testLabels).argMax(1);
    const incorrect = predictions.notEqual(_testLabels).sum().get();
    return (predictions.shape[0] - incorrect) / predictions.shape[0];
  }

  processFeatures(_features) {
    let features = tf.tensor(_features);
